...
void PrintReturnCode()
{
	printf("\n");
	printf("	0	Success.\n");
	printf("	-801	The PacketsGrasper service provider not installed.\n");
	printf("	-802	The PacketsGrasper service provider already installed.\n");
	printf("	-803	Failed to open registry.\n");
	printf("	-804	Failed to save the path of PacketsGrasper service provider.\n");
...
int main(int argc, char* argv[])
{
	__try
	{
		if(argc <= 1 || argc >= 3)
		{
			...
			printf("  -install : used to install PacketsGrasper service provider.\n");
			printf("  -remove  : used to remove the PacketsGrasper service provider.\n");
			return 0;
		}
		...
		if(_tcscmp(argv[1], _T("-install")) == 0)
		{
			...
			_tcscat(sProvider, PACKETSGRASPER_DLL_NAME);
