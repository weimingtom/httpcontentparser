...
int __cdecl main(void)
{
    ...
    //GetModuleFileName(hModule, szFull, sizeof(szFull));
    ...
    printf("Modules:\n");
    pfSymEnumerateModules64(hProcess, SymEnumerateCallback, NULL);
    printf("\n");
    ...
    if (!pfSymGetModuleInfo64(hProcess, (DWORD64)hModule, &modinfo)) {
        ...
        printf("SymGetModuleInfo64(%p, %p) [64] succeeded: %d\n",
                      hProcess, hModule, GetLastError());
        strcpy(szModName, modinfo.ModuleName);
        strcat(szModName, "!");
    ...
    // Look for Target
    strcpy(szFullName, szModName);
    strcat(szFullName, "Target");
    printf("Symbol: [%s]\n", szFullName);
