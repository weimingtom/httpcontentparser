...
STDAPI PingMessage(PCSTR msg, ...)
{
    ...
    vsprintf_s(s_szMessageBuf, arrayof(s_szMessageBuf), msg, args);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
        vsprintf_s(s_szMessageBuf, arrayof(s_szMessageBuf), pszMsg, args);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
    wcscpy_s(wzKey, arrayof(wzKey), pwzKey);
#else
    wcscpy(wzKey, pwzKey);
#endif
                              ...
                              PWCHAR pwzValueName, PWCHAR pwzValue)
{
    ...
    if (pwzSubkey != NULL) {
#ifdef _CRT_INSECURE_DEPRECATE
        wcscat_s(wzKey, arrayof(wzKey), L"\\");
        wcscat_s(wzKey, arrayof(wzKey), pwzSubkey);
#else
        wcscat(wzKey, L"\\");
        wcscat(wzKey, pwzSubkey);
#endif
    }

    if (RegCreateKeyEx(hRootKey, wzKey, 0, NULL, REG_OPTION_NON_VOLATILE,
                       KEY_ALL_ACCESS, NULL, &hKey, NULL) != ERROR_SUCCESS) {
        return FALSE;
    }

    if (pwzValue != NULL) {
        RegSetValueEx(hKey, pwzValueName, 0, REG_SZ,
                      (BYTE *)pwzValue, wcssize(pwzValue));
    }
    RegCloseKey(hKey);
    return TRUE;
}
                              ...
                              PBYTE pbData, ULONG cbData)
{
    ...
    if (pbData != NULL) {
        RegSetValueEx(hKey, pwzValueName, 0, REG_BINARY, pbData, cbData);
    }
    RegCloseKey(hKey);
    return TRUE;
}
...
static void Register(void)
{
...
#ifdef _CRT_INSECURE_DEPRECATE
        wcscpy_s(wzName, arrayof(wzName), pwz + 1);
#else
        wcscpy(wzName, pwz + 1);
#endif
    }
...
#ifdef _CRT_INSECURE_DEPRECATE
        wcscpy_s(wzName, arrayof(wzName), wzModule);
#else
    ...
    // printf("  Class: %ls\n", wzClass);
#ifdef _CRT_INSECURE_DEPRECATE
    wcscpy_s(wzKey, arrayof(wzKey), L"CLSID\\");
    wcscat_s(wzKey, arrayof(wzKey), wzClass);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
    wcscpy_s(wzKey, arrayof(wzKey), L"AppID\\");
    wcscat_s(wzKey, arrayof(wzKey), wzClass);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
    wcscpy_s(wzKey, arrayof(wzKey), L"AppID\\");
    wcscat_s(wzKey, arrayof(wzKey), wzName);
#else
    wcscpy(wzKey, L"AppID\\");
    wcscat(wzKey, wzName);
#endif
...
#ifdef _CRT_INSECURE_DEPRECATE
    wcscpy_s(wzKey, arrayof(wzKey), L"Software\\Microsoft\\Detours\\ProcessorCycles");
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
        swprintf_s(wzValue, arrayof(wzValue), L"%d", cycles);
#else
               ...
               BOOLEAN fToClient, LONG cbPacket, LONG nCount)
{
    ...
    if (hNetwork != NULL) {
        ...
        if (fToClient) {
#ifdef _CRT_INSECURE_DEPRECATE
            swprintf_s(wzKey, arrayof(wzKey), L"ToClient\\%d", cbPacket);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
            swprintf_s(wzKey, arrayof(wzKey), L"ToServer\\%d", cbPacket);
#else
            swprintf(wzKey, L"ToServer\\%d", cbPacket);
#endif
        }
#ifdef _CRT_INSECURE_DEPRECATE
        swprintf_s(wzLatency, arrayof(wzLatency), L"%I64d", msAvg);
#else
    ...
    for (; arg < argc; arg++) {
        if (argv[arg][0] == '-' || argv[arg][0] == '/') {
            ...
            switch (argn[0]) {
              ...
              case '\0':                                // Local Host
#ifdef _CRT_INSECURE_DEPRECATE
                wcscpy_s(wzServers[nServers++], arrayof(wzServers[nServers++]), L"localhost");
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
            wcscpy_s(wzServers[nServers++], arrayof(wzServers[nServers++]), argv[arg]);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
                    wcscpy_s(g_wzServerName, arrayof(g_wzServerName), wzServers[n] + 2);
#else
...
#ifdef _CRT_INSECURE_DEPRECATE
                    wcscpy_s(g_wzServerName, arrayof(g_wzServerName), wzServers[n]);
#else
